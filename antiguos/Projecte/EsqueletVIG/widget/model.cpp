...
void Model::Render(int colorFals, Material matObj)
{
  ...
  for(unsigned int cara = 0; cara < faces.size(); cara++) {
    ...
    if(colorFals == -1){
    //const Material matObj = Scene().matlib.material(faces[cara].material);
    float vk[4];
        ...
        vk[2]= matObj.kd.b;
	vk[3]= matObj.kd.a;
        glMaterialfv(GL_FRONT, GL_DIFFUSE, vk);
	
        vk[0]= matObj.ks.r;
        vk[1]= matObj.ks.g;
        vk[2]= matObj.ks.b;
	vk[3]= matObj.ks.a;
        glMaterialfv(GL_FRONT, GL_SPECULAR, vk);
	//cout << "ks "<< vk[0] << " " << vk[1] << " " << vk[2]<< " a " <<vk[3]<< endl;
             
        vk[0]= matObj.ka.r;
        vk[1]= matObj.ka.g;
        vk[2]= matObj.ka.b;
	vk[3]= matObj.ka.a;
        glMaterialfv(GL_FRONT, GL_AMBIENT, vk);
	//cout << "ka "<< vk[0] << " " << vk[1] << " " << vk[2]<< " a " <<vk[3]<< endl;
        
        glMaterialf(GL_FRONT, GL_SHININESS, matObj.shininess);
	glNormal3f(faces[cara].normal.x, faces[cara].normal.y, faces[cara].normal.z);
	//cout << "normals " << faces[cara].normal.x <<" "<< faces[cara].normal.y <<" "<< faces[cara].normal.z<<endl;
   
    }

    else glColor3ub(colorFals,0,0);
...
Box& Model::calculaTransformedBox(){
	Box nuevaBox;

    	Point p;
	p=nouPunt(vertices[0].coord);
    	nuevaBox=Box(p, p);
    	for (unsigned int i=1; i<vertices.size(); i++){
		//cout <<"PUNTS VELLS "<< vertices[i].coord.x  <<" "<< vertices[i].coord.y <<" " << vertices[i].coord.z << endl;
		p=nouPunt(vertices[i].coord);
...
Point Model::nouPunt(Point p){
	float punt[16];
	GLfloat m[16];
  	int i; 
  	for (i=4; i<16; ++i) punt[i]=0.0;
  	...
  	p.y = m[1];
  	p.z = m[2];
	//cout <<"PUNTS NOUS "<< p.x  <<" "<< p.y <<" " << p.z << endl;
